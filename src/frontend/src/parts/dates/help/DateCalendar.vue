<template>
    <table class="calendar" :class="calendarClass">
        <thead>
            <th class="calendar-nav" @click="prevMonth">
                <i class="fas fa-chevron-left"></i>
            </th>
            <th class="calendar-current" colspan="5">
                <MonthPicker :current="current" @setMonth="setMonth" />
            </th>
            <th class="calendar-nav" @click="nextMonth">
        ...
        <tbody>
            ...
            <tr v-for="(week, i) in currentMonthDays" :key="i">
                <td
                    ...
                    class="date"
                    :class="dayClass(d)"
                    v-bind="isShowTitle(d.date)"
                >
                    <div @mousedown="selectDate(d.date, 'calendar')">
                        {{ d.day }}
                    </div>
                </td>
            </tr>
