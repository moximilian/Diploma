...
class SlotsCRUD(BaseCRUD):
    ...
    def get(self, request_body: RequestBodyOne): 
        ...
        if (slot.repeat_id is None): return slot
    ...
    def list(self, request_body: RequestBodyList): 
        ...
        is_filter_by_creator_id = len([where for where in wheres if where['column'] == 'creator_id']) != 0
        is_filter_asked_by_teacher = len([where for where in wheres if where['column'] == 'role' and where['value'] == 'teacher']) != 0
